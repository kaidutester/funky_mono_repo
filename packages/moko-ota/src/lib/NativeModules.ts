...
export function getMokoBasicModule() {
  if (Platform.OS === 'android') {
    return NativeModules?.MokoModule;
  } else {
    return null;
  }
}

export function initMokoBasicModule() {
  const module = getMokoBasicModule();
  if (module) {
    module.init();
    console.log("init Moko Basic Module");
    return module;
  }
  return null;
}
