...
export async function sendPut(url, data) {
  const [result, resultErr] = await helpers.handle(axios.put(url, data));
...
export async function putFloor(data) {
  console.debug(`putFloor receive input ${helpers.inspect(data)}`);
  const input = processSubmittedFloor(data);
  const url = PRE_CONFIG_APIS.FLOOR;
  console.debug(`putFloor send request with ${helpers.inspect(input)}`);
  const res = await sendPut(url, input);
  return res;
}
