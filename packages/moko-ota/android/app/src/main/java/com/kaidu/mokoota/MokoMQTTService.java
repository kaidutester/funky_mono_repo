...
public class MokoMQTTService extends Service {
    ...
    // return exisitng client or create a new one by given inputs
    public MqttAndroidClient getClient(String host, String port, String clientId, boolean tlsConnection) {
        if (this.mqttAndroidClient != null && this.mqttAndroidClient instanceof MqttAndroidClient) {
            return this.mqttAndroidClient;
        } else {
            return this.createClient(host, port, clientId, tlsConnection);
        }
    ...
    public IMqttToken disconnectMqttServer() {
        if (!isConnected())
            return null;
