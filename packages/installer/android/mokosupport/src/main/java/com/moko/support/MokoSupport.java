...
//            LogModule.w("connDevice: 地址为空");
            Log.d("MokoModule", "connDevice: 地址为空");
            return;
        }
        if (!isBluetoothOpen()) {
//            LogModule.w("connDevice: 蓝牙未打开");
            Log.d("MokoModule", "connDevice: 蓝牙未打开");
            return;
        }
        ...
        if (device != null) {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    Log.d("MokoModule", "connDevice: 开始尝试连接");
                    mBluetoothGatt = (new BleConnectionCompat(context)).connectGatt(device, false, gattCallback);
                    Log.d("MokoModule", "connDevice: set mBluetoothGatt");
                }
            });
        } else {
            LogModule.w("获取蓝牙设备失败");
            Log.d("MokoModule", "获取蓝牙设备失败");
        }
    }
    ...
    public void sendOrder(OrderTask... orderTasks) {
        ...
        if (!isSyncData()) {
            for (OrderTask ordertask : orderTasks) {
                ...
                Log.d("MokoModule", "sendOrder: " + ordertask.order.getOrderName());
            }
            executeTask(null);
        } else {
            for (OrderTask ordertask : orderTasks) {
    ...
    public synchronized void executeTask(MokoOrderTaskCallback callback) {
        if (callback != null && !isSyncData()) {
            Log.d("MokoModule", "mQueue is all finished");
            callback.onOrderFinish();
            return;
        }
        if (mQueue.isEmpty()) {
            Log.d("MokoModule", "mQueue is Empty");
            return;
        }
        final OrderTask orderTask = mQueue.peek();
        if (mBluetoothGatt == null) {
            LogModule.i("executeTask : BluetoothGatt is null");
            Log.d("MokoModule", "executeTask : BluetoothGatt is null");
            return;
        }
        if (orderTask == null) {
            LogModule.i("executeTask : orderTask is null");
            Log.d("MokoModule", "executeTask : orderTask is null");
            return;
        }
    ...
    public synchronized boolean isSyncData() { //has data in queue
    ...
    public void disConnectBle() {
        Log.d("MokoModule", "MokoSupport: disConnectBle");
    ...
    public void onDescriptorWrite() {
        ...
        mQueue.poll();
        Log.d("MokoModule", "mQueue poll a order");
    ...
    public void pollTask() {
        if (mQueue != null && !mQueue.isEmpty()) {
            ...
            mQueue.poll();
            Log.d("MokoModule", "移除" + orderTask.order.getOrderName());
        }
    }
    ...
    private void sendWriteOrder(OrderTask orderTask, final MokoCharacteristic mokoCharacteristic) {
        ...
        Log.d("MokoModule", "app to device write : " + orderTask.orderType.getName());
    ...
    private void sendWriteNoResponseOrder(OrderTask orderTask, final MokoCharacteristic mokoCharacteristic) {
        ...
        Log.d("MokoModule", "app to device write no response : " + orderTask.orderType.getName());
        LogModule.i(MokoUtils.bytesToHexString(orderTask.assemble()));
        mokoCharacteristic.characteristic.setValue(orderTask.assemble());
