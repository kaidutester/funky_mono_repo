...
import { isFilledArray, trimObjectProperties, inspect, replaceEmptyStringWithNull } from '@kaidu/shared/utils';

/**
 * @description if all valid, return valid, if some are valid, return some, if none are valid, return invalid
 * @param  {any[]} arr an array with {validity}
 * @returns string the validity value of this whole array
 */
function checkValidityOfArray(arr: any[]): Validity {
  const allValid = arr.every((item) => item.validity === Validity.VALID);
  if (allValid) {
    return Validity.VALID;
  } else {
    const someValid = arr.some(
      (item) =>
        item.validity === Validity.VALID ||
        item.validity === Validity.SOME_VALID
    );
    if (someValid) {
      return Validity.SOME_VALID;
    } else {
...
export function processSubmittedCustomerDeviceDataItem(
  ...
  const trimmed = trimObjectProperties(input);
...
export function processSubmittedFloorPlan(
  ...
  if (collected?.svg) {
    ...
    console.debug(
      `submitted floor plan (except svg) : ${inspect(
